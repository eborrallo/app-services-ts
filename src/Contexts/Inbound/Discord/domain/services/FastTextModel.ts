import fs from 'fs';
import { Language, getLanguageIndexByCode } from '../helpers/Languages';
...
export class FastTextModel {
  ...
  static async load(modelPath: string) {
    ...
    const fastTextModuleJs = fs.readFileSync(__dirname + '/../resources/fasttext/fasttext_wasm.js').toString('utf-8');
    const contentWasm = fs.readFileSync(__dirname + '/../resources/fasttext/fasttext_wasm.wasm');
    const fastTextModuleWasm = contentWasm.buffer.slice(
      contentWasm.byteOffset,
      contentWasm.byteOffset + contentWasm.byteLength
    );

    const contentModel = fs.readFileSync(modelPath);

    const model = new Uint8Array(
      contentModel.buffer.slice(contentModel.byteOffset, contentModel.byteOffset + contentModel.byteLength)
    );
...
export class FastTextLID176Model {
  ...
  static async load() {
    return new FastTextLID176Model(await FastTextModel.load(__dirname + '/../resources/models/lid.176.ftz'));
  }
}
